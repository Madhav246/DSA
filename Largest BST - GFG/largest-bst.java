...
class NodeValue {
     ...
     public NodeValue(int _minNode, int _maxNode, int _maxSize){
         maxNode = _maxNode;
         minNode = _minNode;
         maxSize = _maxSize;
     }
...
class Solution{
    static NodeValue solve(Node root){
        ...
        if(left.maxNode < root.data && right.minNode > root.data){
            return new NodeValue(Math.min(left.minNode, root.data), 
                                Math.max(right.maxNode, root.data), left.maxSize + right.maxSize + 1);
        }
